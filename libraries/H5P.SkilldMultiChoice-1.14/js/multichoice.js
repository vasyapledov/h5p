...
<li class="h5p-answer" order-id="<%= answers[i].originalOrder %>" role="<%= answers[i].role %>" tabindex="<%= answers[i].tabindex %>" aria-checked="<%= answers[i].checked %>" data-id="<%= i %>"> \
...
<% } %> \
  ...
  var addFeedback = function ($element, feedback) {
    $feedbackDialog = $('<div class="h5p-feedback-dialog"> \
...
/span>';
  ...
  params.label = 'h5p-mcq' + H5P.SkilldMultiChoice.counter;

  /**
   * Pack the current state of the interactivity into a object that can be serialized.
   *
   * @public
   */
  this.getCurrentState = function () {
    ...
    if (!idMap) {
      ...
      // The answers have been shuffled and must be mapped back to their original ID.
      state.answers = [];
      for (var i = 0; i < params.userAnswers.length; i++) {
